import React from 'react';
import { useNavigate, useLocation } from 'react-router-dom';
import {
  Home,
  History,
  MessageSquareWarning,
  Calendar,
  Tag,
  Info
} from 'lucide-react';

const BottomNav = () => {
  const navigate = useNavigate();
  const location = useLocation();
  
  // ✅ LOGIC: Check login status
  const user = JSON.parse(localStorage.getItem('user'));

  // Define navigation items with protection flags
  const allNavItems = [
    { name: 'Home', icon: Home, path: '/' },
    { name: 'Schedule', icon: Calendar, path: '/schedule' },
    { name: 'Price', icon: Tag, path: '/prices' },
    { name: 'History', icon: History, path: '/history', protected: true },
    { name: 'Help', icon: MessageSquareWarning, path: '/complaint', protected: true },
    { name: 'About', icon: Info, path: '/about' },
  ];

  // ✅ LOGIC: Filter items - guests only see unprotected items
  const visibleNavItems = user 
    ? allNavItems 
    : allNavItems.filter(item => !item.protected);

  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-slate-900/95 border-t border-gray-100 dark:border-slate-800 z-50 backdrop-blur-xl shadow-[0_-4px_16px_rgba(0,0,0,0.08)] px-2 pt-2 pb-[calc(env(safe-area-inset-bottom)+0.5rem)]">
      <div className="flex justify-around items-center gap-1">
        {visibleNavItems.map((item) => {
          // Logic: Highlight the active button
          const isActive = item.path === '/' 
            ? location.pathname === '/' 
            : location.pathname.startsWith(item.path);

          return (
            <button
              key={item.name}
              onClick={() => navigate(item.path)}
              className={`relative flex flex-col items-center justify-center flex-1 min-w-[52px] transition-all duration-200 active:scale-90 ${
                isActive ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400'
              }`}
            >
              <div className={`p-1.5 rounded-xl transition-colors ${
                isActive ? 'bg-indigo-50 dark:bg-indigo-900/30' : 'bg-transparent'
              }`}>
                <item.icon size={20} strokeWidth={isActive ? 2.5 : 2} />
              </div>

              {/* ✅ DESIGN: Maintained established fonts and styling */}
              <span className={`text-[10px] mt-1 font-bold truncate w-full text-center transition-opacity ${
                isActive ? 'opacity-100' : 'opacity-70'
              }`}>
                {item.name}
              </span>

              {isActive && (
                <div className="absolute -bottom-1 w-1 h-1 bg-indigo-600 dark:bg-indigo-400 rounded-full" />
              )}
            </button>
          );
        })}
      </div>
    </div>
  );
};

export default BottomNav;